...
const STATUS_STYLES: Record<TransportStatus, string> = {
  ...
  assigned: 'bg-neutral-500/15 text-neutral-300 border-neutral-500/30',
    ...
    <div className="space-y-6">
      ...
      {delayedRows.length > 0 && (
        <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 space-y-2">
          ...
          {delayedRows.map((d) => (
            <div key={d.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-neutral-300">
              <span className="font-mono font-bold text-neutral-200">{d.trip_id}</span>
              <span>{d.vehicle?.name || 'No vehicle'} · {d.driver?.name || 'No driver'}</span>
              <span className="text-rose-300">{d.delay_reason}</span>
              <button onClick={() => setStatusDialog({ tripId: d.trip_id, to: d.pre_delay_status === 'en_route' ? 'en_route' : 'assigned' })} className="text-neutral-200 font-bold hover:underline">Resolve →</button>
              <button onClick={() => { setNotifyDialog({ tripId: d.trip_id }); setFormEvent('delay'); setNotifyResult(null); }} className="text-neutral-300 font-bold hover:underline">Notify traveler</button>
            </div>
          ))}
        </div>
      )}
      ...
      {view === 'dispatch' && (
        <>
          ...
          {loading ? (
            <div className="text-center py-12 text-neutral-500 text-sm">Loading dispatch board...</div>
          ) : filtered.length === 0 ? (
            ...
            <div className="space-y-3">
              {filtered.map(({ trip, assignment: a, status }) => {
                ...
                return (
                <div key={trip.id} className={`bg-neutral-900 border rounded-2xl p-5 space-y-3 ${status === 'delayed' ? 'border-rose-500/50' : 'border-neutral-800'}`}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <button onClick={() => openAssign(trip.id)} className="px-3 py-1.5 bg-white hover:bg-neutral-200 text-black text-xs font-bold rounded-lg flex items-center gap-1">
                      ...
                      <button onClick={() => { setNotifyDialog({ tripId: trip.id }); setFormEvent(status === 'delayed' ? 'delay' : 'assignment'); setNotifyResult(null); setFormNote(''); }} className="px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 text-xs font-bold rounded-lg border border-neutral-700 flex items-center gap-1">
                        <Bell className="w-3 h-3" /> Notify
                      </button>
                      <button onClick={() => onSelectTrip(trip.id)} className="px-3 py-1.5 text-neutral-400 hover:text-white text-xs font-semibold">Open →</button>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                    <div className="bg-neutral-950/60 rounded-xl p-2.5 border border-neutral-800/80">
                      <div className="text-[10px] uppercase text-neutral-500 font-bold flex items-center gap-1"><Car className="w-3 h-3" /> Vehicle</div>
                    ...
                    <div className="bg-neutral-950/60 rounded-xl p-2.5 border border-neutral-800/80">
                      <div className="text-[10px] uppercase text-neutral-500 font-bold flex items-center gap-1"><Users className="w-3 h-3" /> Driver</div>
                      <div className="text-neutral-200 font-semibold truncate">{a?.driver?.name || 'Unassigned'}</div>
                      <div className="text-neutral-500 text-[11px]">{a?.driver?.phone || 'assign a driver'}</div>
                    </div>
                    <div className="bg-neutral-950/60 rounded-xl p-2.5 border border-neutral-800/80">
                      <div className="text-[10px] uppercase text-neutral-500 font-bold flex items-center gap-1"><MapPin className="w-3 h-3" /> Route</div>
                      <div className="text-neutral-200 font-semibold truncate">{a?.origin || trip.origin || '?'} → {a?.destination || trip.destination?.name || '?'}</div>
                      <div className="text-neutral-500 text-[11px] flex items-center gap-1"><Clock className="w-3 h-3" /> {fmtDT(a?.pickup_at)} → {fmtDT(a?.dropoff_at)}</div>
                    </div>
                    <div className="bg-neutral-950/60 rounded-xl p-2.5 border border-neutral-800/80">
                      <div className="text-[10px] uppercase text-neutral-500 font-bold">Journey</div>
      ...
      {view === 'fleet' && (
        <div className="space-y-3">
          <div className="flex justify-end">
            <button onClick={() => { setFleetDialog('vehicle'); setFormName(''); setFormReg(''); setFormType('private_cab'); setFormCapacity('4'); }} className="px-3 py-2 bg-white hover:bg-neutral-200 text-black text-xs font-bold rounded-xl flex items-center gap-1">
              <Plus className="w-3.5 h-3.5" /> Onboard Vehicle
            </button>
          </div>
          {vehicles.length === 0 && !loading ? (
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-8 text-center text-neutral-400 text-xs">No vehicles in dispatch inventory.</div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              {vehicles.map((v) => {
                const linked = assignments.filter((a) => a.vehicle_id === v.id && ['assigned', 'en_route', 'delayed'].includes(a.status));
                return (
                  <div key={v.id} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-[11px] font-bold text-neutral-200 bg-neutral-950 px-2 py-0.5 rounded border border-neutral-800">{v.registration_number}</span>
                      <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${v.is_active ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30' : 'bg-neutral-800 text-neutral-400 border-neutral-700'}`}>
                        {v.is_active ? 'active' : 'inactive'}
                      </span>
                    </div>
                    <div className="text-sm font-bold text-white">{v.name}</div>
                    <div className="text-xs text-neutral-400">{v.vehicle_type} · {v.capacity} seats</div>
      ...
      {view === 'drivers' && (
        <div className="space-y-3">
          <div className="flex justify-end">
            <button onClick={() => { setFleetDialog('driver'); setFormName(''); setFormPhone(''); setFormLicense(''); }} className="px-3 py-2 bg-white hover:bg-neutral-200 text-black text-xs font-bold rounded-xl flex items-center gap-1">
              <Plus className="w-3.5 h-3.5" /> Onboard Driver
            </button>
          </div>
          {drivers.length === 0 && !loading ? (
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-8 text-center text-neutral-400 text-xs">No drivers on the roster.</div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              {drivers.map((d) => {
                const linked = assignments.filter((a) => a.driver_id === d.id && ['assigned', 'en_route', 'delayed'].includes(a.status));
                return (
                  <div key={d.id} className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <div className="text-sm font-bold text-white">{d.name}</div>
                      <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${d.is_active ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30' : 'bg-neutral-800 text-neutral-400 border-neutral-700'}`}>
                        {d.is_active ? 'active' : 'inactive'}
                      </span>
                    </div>
                    <div className="text-xs text-neutral-400">{d.phone || 'No phone'} {d.license_number ? `· Lic ${d.license_number}` : ''}</div>
                    <div className="text-xs text-neutral-500">
                      {linked.length === 0 ? 'No active commitments.' : `Committed to: ${linked.map((l) => l.trip_id).join(', ')}`}
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-700 rounded-2xl p-6 max-w-lg w-full space-y-4 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setAssignDialog(null)} className="p-1 text-neutral-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2 sm:col-span-1">
                <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Vehicle (live inventory)</label>
                <select value={formVehicleId} onChange={(e) => setFormVehicleId(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white">
              ...
              <div className="col-span-2 sm:col-span-1">
                <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Driver (live roster)</label>
                <select value={formDriverId} onChange={(e) => setFormDriverId(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white">
                  <option value="">— None —</option>
                  {drivers.map((d) => (
              ...
              <div>
                <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Origin</label>
                <input type="text" value={formOrigin} onChange={(e) => setFormOrigin(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
              </div>
              <div>
                <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Destination</label>
                <input type="text" value={formDestination} onChange={(e) => setFormDestination(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
              </div>
              <div>
                <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Pickup</label>
                <input type="datetime-local" value={formPickup} onChange={(e) => setFormPickup(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
              </div>
              <div>
                <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Drop-off</label>
                <input type="datetime-local" value={formDropoff} onChange={(e) => setFormDropoff(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
              </div>
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => setAssignDialog(null)} className="px-4 py-2 rounded-xl bg-neutral-800 text-neutral-300 text-xs font-bold">Cancel</button>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-700 rounded-2xl p-6 max-w-md w-full space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setStatusDialog(null)} className="p-1 text-neutral-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            {statusDialog.to === 'delayed' && (
              <>
                <label className="block text-xs text-neutral-400 font-bold uppercase">Delay reason (required)</label>
                <textarea value={formReason} onChange={(e) => setFormReason(e.target.value)} rows={3} placeholder="e.g. Landslide on NH-3, revised pickup +2h" className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
              </>
            )}
            <div className="flex justify-end gap-2">
              <button onClick={() => setStatusDialog(null)} className="px-4 py-2 rounded-xl bg-neutral-800 text-neutral-300 text-xs font-bold">Cancel</button>
              <button onClick={submitStatus} disabled={submitting} className="px-4 py-2 rounded-xl bg-white hover:bg-neutral-200 text-black text-xs font-bold disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-700 rounded-2xl p-6 max-w-md w-full space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setNotifyDialog(null)} className="p-1 text-neutral-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <label className="block text-xs text-neutral-400 font-bold uppercase">Event</label>
            <select value={formEvent} onChange={(e) => setFormEvent(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white">
              {NOTIFY_EVENTS.map((ev) => (
                <option key={ev} value={ev}>{ev.replace('_', ' ')}</option>
              ))}
            </select>
            <label className="block text-xs text-neutral-400 font-bold uppercase">Operator note (optional)</label>
            <textarea value={formNote} onChange={(e) => setFormNote(e.target.value)} rows={3} placeholder="e.g. Driver will call 30 min before pickup" className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
            {notifyResult && (
              <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs">{notifyResult}</div>
            )}
            <div className="flex justify-end gap-2">
              <button onClick={() => setNotifyDialog(null)} className="px-4 py-2 rounded-xl bg-neutral-800 text-neutral-300 text-xs font-bold">Close</button>
              <button onClick={submitNotify} disabled={submitting} className="px-4 py-2 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-white text-xs font-bold disabled:opacity-50 flex items-center gap-1">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-700 rounded-2xl p-6 max-w-md w-full space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setFleetDialog(null)} className="p-1 text-neutral-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            ...
            {fleetDialog === 'vehicle' ? (
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Registration number</label>
                  <input type="text" value={formReg} onChange={(e) => setFormReg(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
                </div>
                <div>
                  <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Type</label>
                ...
                <div>
                  <label className="block text-xs text-neutral-400 font-bold uppercase mb-1">Capacity</label>
                  <input type="number" min={1} value={formCapacity} onChange={(e) => setFormCapacity(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
                </div>
              </div>
            ) : (
              <>
                ...
                <label className="block text-xs text-neutral-400 font-bold uppercase">License number</label>
                <input type="text" value={formLicense} onChange={(e) => setFormLicense(e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-xs text-white" />
              </>
            )}
            <div className="flex justify-end gap-2">
              <button onClick={() => setFleetDialog(null)} className="px-4 py-2 rounded-xl bg-neutral-800 text-neutral-300 text-xs font-bold">Cancel</button>
              <button onClick={submitFleet} disabled={submitting} className="px-4 py-2 rounded-xl bg-white hover:bg-neutral-200 text-black text-xs font-bold disabled:opacity-50">
